...
def forecast(station):
    ...
    # get and set date for calculate forecast based on this TODO: forecast_date now is static
    if station.state_of_data in [1, 3]:
        try:
            globals_vars.forecast_date = int(config_run.settings['forecast_date'])
        finally:
            if not (1 <= globals_vars.forecast_date <= 12):
                console.msg_error_configuration('forecast_date',
                    _("Trimester forecast '{0}' is invalid, "
                      "must be a month valid number (1-12)")
                    .format(globals_vars.forecast_date), show_settings=False)
    if station.state_of_data in [2, 4]:
        try:
            globals_vars.forecast_date = config_run.settings['forecast_date'].replace('-', '/')
            globals_vars.forecast_date = globals_vars.forecast_date.split('/')
            globals_vars.forecast_date[0] = int(globals_vars.forecast_date[0])
            globals_vars.forecast_date[1] = int(globals_vars.forecast_date[1])
        except:
            console.msg_error_configuration('forecast_date',
        ...
        if not (1 <= globals_vars.forecast_date[0] <= 12):
            console.msg_error_configuration('forecast_date',
                _("Month for forecast process '{0}' is invalid, \n"
                  "must be a month valid number (1-12)")
                .format(globals_vars.forecast_date[0]), show_settings=False)
        if globals_vars.forecast_date[1] not in station.range_analysis_interval:
            console.msg_error_configuration('forecast_date',
                _("Start day (month/day) for forecast process '{0}'\nis invalid, "
                  "must be a valid start day based on\nrange analysis "
                  "interval, the valid start days for\n{1} are: {2}")
                .format(globals_vars.forecast_date[1], globals_vars.analysis_interval_i18n,
            ...
            for column in range(3):
                for row in range(3):
                    if not config_run.settings['risk_analysis'] or\
                       station.is_sig_risk_analysis[lag][globals_vars.forecast_date - 1][_iter] == _('yes'):
                        items_CT[order_CT[_iter]] \
                            = station.contingencies_tables_percent[lag][globals_vars.forecast_date - 1][column][row] / 100.0
                        _iter += 1

        if station.state_of_data in [2, 4]:
            month = globals_vars.forecast_date[0]
            day = station.range_analysis_interval.index(globals_vars.forecast_date[1])
            _iter = 0
            for column in range(3):
                for row in range(3):
                    if not config_run.settings['risk_analysis'] or\
