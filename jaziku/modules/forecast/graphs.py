...
def forecast_graphs(station):
    ...
    for lag in env.config_run.settings['lags']:
        ...
        if env.config_run.settings['class_category_analysis'] == 3:
            ...
            if True in [isnan(value) for value in values_pie]:
                ...
                pyplot.legend(tuple(pie_plot[0]), labels, loc=(0.96, 0.4), borderaxespad=-3, shadow=False,
                              fancybox=True, fontsize=10, labelspacing=0.3)
            ...
            image_dir_save \
                = os.path.join(station.forecast_dir, _('prob_of_{0}_lag_{1}_{2}_({3}-{4}).png')
                               .format(station.var_D.type_series, lag,
        ...
        if env.config_run.settings['class_category_analysis'] == 7:
            ...
            if True in [isnan(value) for value in values_pie]:
                ...
                for wedges in pie_plot[0]:
                    wedges.set_linewidth(0)
                pyplot.legend(tuple(pie_plot[0]), labels, loc=(0.96, 0.27), borderaxespad=-3, shadow=False,
                              fancybox=True, fontsize=9, labelspacing=0.3)
    ...
    if len(env.config_run.settings['lags']) != 1:
        ...
        dpi = 100.0
        mosaic_plots = pyplot.figure(
            figsize=((image_width * len(env.config_run.settings['lags'])) / dpi, image_height / dpi))
        ...
        for lag in env.config_run.settings['lags']:
            ...
            if env.var_D.is_n_monthly():
                ...
                station.var_D.specific_values \
                    = time_series.get_specific_values(station, 'var_D', lag,
                                                      env.config_run.settings['forecast_date']['month'])
            ...
            if True in [isnan(value) for value in values_pie]:
                ...
                for wedges in pie_plot[0]:
                    wedges.set_linewidth(0)
                pyplot.legend(tuple(pie_plot[0]), labels, loc=(0.96, 0.2), borderaxespad=-3, shadow=False,
                              fancybox=True, fontsize=11, labelspacing=0.3)
