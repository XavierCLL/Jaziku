...
def configuration_run():
    ...
    env.config_run.settings['categories_labels_var_I'] = format_labels(
        env.config_run.settings['categories_labels_var_I'])
    ...
    if env.config_run.settings['mode_calculation_series_D'] == 'default':
        if env.var_D.TYPE_SERIES in env.var_D.MODE_CALCULATION_SERIES:
            env.config_run.settings['mode_calculation_series_D'] = \
            env.var_D.MODE_CALCULATION_SERIES[env.var_D.TYPE_SERIES][0]
        else:
            env.config_run.settings['mode_calculation_series_D'] = 'mean'
        env.globals_vars.input_settings["mode_calculation_series_D"] = env.config_run.settings[
            'mode_calculation_series_D']
    elif env.config_run.settings['mode_calculation_series_D'] in ['accumulate', 'mean']:
        if env.var_D.TYPE_SERIES in env.var_D.MODE_CALCULATION_SERIES:
            if env.config_run.settings['mode_calculation_series_D'] not in env.var_D.MODE_CALCULATION_SERIES[
        ...
        env.globals_vars.input_settings["mode_calculation_series_D"] = colored.green(
            env.config_run.settings['mode_calculation_series_D'])
    else:
        console.msg_error_configuration('mode_calculation_series_D',
                                        _("The 'mode_calculation_series_D' is invalid, the options are:\n"
    ...
    if env.config_run.settings['limits_var_D']['below'] == "default":
        ...
        try:
            env.config_run.settings['limits_var_D']['below'] = input.to_float(
                env.config_run.settings['limits_var_D']['below'])
        except:
            console.msg_error_configuration('limits_var_D',
                                            (_("Problem with particular range validation for "
                                               "dependent\nvariable: '{0}' this must be "
                                               "a valid number, 'none' or 'default'.").format(
                                                env.config_run.settings['limits_var_D']['below'])))
    ...
    if env.config_run.settings['limits_var_D']['above'] == "default":
        ...
        try:
            env.config_run.settings['limits_var_D']['above'] = input.to_float(
                env.config_run.settings['limits_var_D']['above'])
        except:
            console.msg_error_configuration('limits_var_D',
                                            (_("Problem with particular range validation for "
                                               "dependent\nvariable: '{0}' this must be "
    ...
    if not env.config_run.settings["thresholds_var_D"] == "default" and \
                    len(env.config_run.settings["thresholds_var_D"]) != env.config_run.settings[
                'class_category_analysis'] - 1:
        console.msg_error_configuration('thresholds_var_D',
                                        ...
                                        .format(env.config_run.settings['class_category_analysis'],
                                                env.config_run.settings['class_category_analysis'] - 1))
    ...
    if env.config_run.settings['mode_calculation_series_I'] == 'default':
        if env.var_I.TYPE_SERIES in env.var_I.MODE_CALCULATION_SERIES:
            ...
            env.var_I.MODE_CALCULATION_SERIES[env.var_I.TYPE_SERIES][0]
        else:
            env.config_run.settings['mode_calculation_series_I'] = 'mean'
        env.globals_vars.input_settings["mode_calculation_series_I"] = env.config_run.settings[
            'mode_calculation_series_I']
    elif env.config_run.settings['mode_calculation_series_I'] in ['accumulate', 'mean']:
        if env.var_I.TYPE_SERIES in env.var_I.MODE_CALCULATION_SERIES:
            if env.config_run.settings['mode_calculation_series_I'] not in env.var_I.MODE_CALCULATION_SERIES[
        ...
        env.globals_vars.input_settings["mode_calculation_series_I"] = colored.green(
            env.config_run.settings['mode_calculation_series_I'])
    else:
        console.msg_error_configuration('mode_calculation_series_I',
                                        _("The mode_calculation_series_I is wrong, the options are:\n"
                                          "default, accumulate or mean"))
    ...
    if env.config_run.settings['limits_var_I']['below'] == "default":
        ...
        try:
            env.config_run.settings['limits_var_I']['below'] = input.to_float(
                env.config_run.settings['limits_var_I']['below'])
        except:
    ...
    if env.config_run.settings['limits_var_I']['above'] == "default":
        ...
        try:
            env.config_run.settings['limits_var_I']['above'] = input.to_float(
                env.config_run.settings['limits_var_I']['above'])
        except:
            console.msg_error_configuration('limits_var_I',
                                            (_("Problem with particular range validation for "
                                               "independent\nvariable: '{0}' this must be "
        ...
        if env.config_run.settings["type_var_I"] not in env.var_I.INTERNAL_TYPES:
            console.msg_error_configuration('path_to_file_var_I',
                                            _("The 'path_to_file_var_I' is defined as 'internal' but the\n"
                                              ...
                                              "a valid path to file var I.").format(
                                                env.config_run.settings["type_var_I"]))
    else:
    ...
    if not env.config_run.settings["thresholds_var_I"] == "default" and \
                    len(env.config_run.settings["thresholds_var_I"]) != env.config_run.settings[
                'class_category_analysis'] - 1:
    ...
    if env.config_run.settings['forecast_process']:
        ...
        for lag in [str(lag) for lag in env.globals_vars.ALL_LAGS]:
            try:
                ...
                if env.config_run.settings['class_category_analysis'] == 3:
                    ...
                    if False in [isinstance(value, (float, int)) for value in
                                 array.clean(env.config_run.settings['forecast_var_I_lag_' + lag])]:
                        raise ValueError('3')
                    ...
                    if len(array.clean(env.config_run.settings['forecast_var_I_lag_' + lag])) == 3:
                        ...
                        env.config_run.settings['forecast_var_I_lag_' + lag] = array.clean(
                            env.config_run.settings['forecast_var_I_lag_' + lag])
                        env.globals_vars.input_settings['forecast_var_I_lag_' + lag] = env.config_run.settings[
                            'forecast_var_I_lag_' + lag]
                    else:
                        raise ValueError('3')
                if env.config_run.settings['class_category_analysis'] == 7:
                    # check if all values are valid (float or int)
            ...
            if isinstance(env.config_run.settings['forecast_date'], list):
                ...
                if env.config_run.settings['analysis_interval'] in ['monthly']:
                    ...
                    if len(env.config_run.settings['forecast_date']) == 2:
                        env.config_run.settings['forecast_date'] = input.bimonthly_char2int(
                            env.config_run.settings['forecast_date'])
                    elif len(env.config_run.settings['forecast_date']) == 3:
                        env.config_run.settings['forecast_date'] = input.trimonthly_char2int(
                            env.config_run.settings['forecast_date'])
        ...
        env.globals_vars.input_settings["forecast_date"] = colored.green(
            env.config_run.settings['forecast_date']['text'])
...
def grids_list():
    ...
    for grid in maps.Grid.all_grids:
        ...
        if not grid.latitude == "internal":
            if isinstance(grid.latitude, list) and len(grid.latitude) == 2 and not False in [isinstance(x, (int, float))
                                                                                             for x in grid.latitude]:
        ...
        if not grid.longitude == "internal":
            if isinstance(grid.longitude, list) and len(grid.longitude) == 2 and not False in [
                isinstance(x, (int, float)) for x in grid.longitude]:
    ...
    for station in stations_list:
        if station.code in list_codes and station.name in list_names:
            console.msg_error_line_stations(station,
                                            _("The combination of the code and name of the station can't repeat,\n"
